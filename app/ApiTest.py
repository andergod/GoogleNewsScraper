...
async def solve_captcha_sync(num_requests, cap_monster_client, recaptcha2request):
    return [
        await cap_monster_client.solve_captcha(recaptcha2request)
...
def test_using_package():
    ...
    recaptcha2request = RecaptchaV2ProxylessRequest(
        websiteUrl=WEBPAGE_URL, websiteKey=WEBPAGE_KEY
    )
    ...
    sync_responses = asyncio.run(
        solve_captcha_sync(nums, cap_monster_client, recaptcha2request)
    )
    print(
